...
import org.taktik.exceptions.exception.NotAnMPEGFileException;
import org.taktik.mpegts.MTSPacket;
import org.taktik.mpegts.sources.data.mpeg.descriptors.*;
import org.taktik.mpegts.sources.data.mpeg.psi.PMTs;
import org.taktik.mpegts.sources.data.mpeg.psi.PMTsection;
import org.taktik.mpegts.sources.data.mpeg.psi.TDTsection;
import org.taktik.utils.OffsetHelper;
import org.taktik.utils.PositionPushbackInputStream;
import org.taktik.utils.ProgressMonitorLargeInputStream;
...
import static org.taktik.mpegts.Constants.TS_MARKER;
import static org.taktik.mpegts.sources.data.mpeg.descriptors.Descriptor.findGenericDescriptorsInList;
import static org.taktik.utils.Utils.*;

/**
 * TransportStream is responsible for parsing a file containing a transport stream, dividing it into 188 byte {@link MTSPacket}, and handing them over to the correct PID.
 *
 */
...
public class TransportStream {
	...
	private static boolean usesPacketLength(final int possiblePacketLength, final RandomAccessFile randomAccessFile) throws IOException {
		...
		do{
			...
			while ((b != TS_MARKER)&&(startPos<MAX_SEARCH_BYTES)){
			...
			for (int i = 1; (i < CONSECUTIVE_PACKETS) && seqFound; i++) {
				...
				seqFound &= (randomAccessFile.read() == TS_MARKER);
			}
			if(seqFound){
				return true;
			}
	...
	/*private void processPacket(MTSPacket packet) {
		final short pid = (short) packet.getPid();
	...
	private static short addPIDFlags(MTSPacket packet, final short pid) {
		short pidFlags = pid;
...
/*
	public void parsePidStreams(final Map<Integer, GeneralPidHandler> toParsePids) throws IOException {
		...
		try (final RandomAccessFile randomAccessFile = new RandomAccessFile(file, "r")){
			for(int t=0; t<no_packets;t++){
				...
				if(handler!=null){
					MTSPacket packet = readPacket(t, randomAccessFile);
	...
	private static int getAncillaryDataIdentifier(final PMTsection.Component component) {
		int ancillaryData = 0;
	...
	public PMTsection getPMTforPID(final int thisPID) {
		...
		for (final PMTsection[] pmTsections : pmts) {
			...
			for(final PMTsection.Component component :pmt.getComponentenList()){
	...
	/*public MTSPacket getTSPacket(final int packetNo){
	...
	/*private MTSPacket readPacket(final int packetNo, final RandomAccessFile randomAccessFile)
			throws IOException {
		MTSPacket packet = null;
		...
		if(bytesRead==packetLength){
			packet = new MTSPacket(buf, packetNo,this);
	...
	/*public Iterator<MTSPacket> getTSPacketsIterator(final int pid, final int flags){
		return  getTSPacketsIterator(pid, flags, 0, getNo_packets());
	}*/

	/*public Iterator<MTSPacket> getTSPacketsIterator(final int pid, final int flags, final int start, final int end){

		final Iterator<MTSPacket> iter = new Iterator<MTSPacket>(){
			...
			@Override
			public MTSPacket next() {
				final MTSPacket packet = getTSPacket(pos);
				pos = findNext(pos);
				return packet;
			}
