...
public class MTSPacket extends PacketSupport {
    ...
    synchronized public PesHeader getPesHeader(){
        if(pesHeader==null){
            if(this.isContainsPayload()&&this.isPayloadUnitStartIndicator()){
                int payloadStart = 4;
                if(this.isAdaptationFieldExist()&&this.getAdaptationField()!=null){
                    payloadStart = 5+this.getAdaptationField().getAdaptationFieldLength();
                }
                byte[] arr = new byte[this.getBuffer().remaining()];
                this.getBuffer().get(arr,0,arr.length);
                pesHeader = new PesHeader(arr, payloadStart);
            }
        }
        return pesHeader;
    }
	...
	public static class AdaptationField {
		...
		private int adaptationFieldLength = -1;
		private PCR pcr;							// PCR
		...
		public static class PCR {
			...
			public long extension;					// 9 bits
			public byte reserved;					// 6 bits

			public PCR(AdaptationField field, long base, long extension, byte reserved) {
				this.base = base;
	...
	public MTSPacket(ByteBuffer buffer,long packetCount) {
		super(buffer);
	...
	protected void write() {
		...
		if (adaptationFieldExist) {
			if (adaptationField == null) {
				buffer.put((byte) 0);
			} else {			adaptationField.write(buffer, payloadLength);
			}
		}
	...
	protected void parse() {
		...
		if (adaptationFieldExist) {
			...
			if (adaptationFieldLength != 0) {
				...
				adaptationField = new MTSPacket.AdaptationField(this, discontinuityIndicator, randomAccessIndicator, elementaryStreamPriorityIndicator, pcrFlag, opcrFlag, splicingPointFlag, transportPrivateDataFlag, adaptationFieldExtensionFlag, pcr, opcr, spliceCountdown, privateData, extension,adaptationFieldLength);
			}
		}
